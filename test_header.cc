...
TEST_F(HeaderTest, setHeader) {
  ...
  ASSERT_TRUE(h_->thePacket().type == std::bitset<2> b1(3));

  //test window
  h_->setWindow(3); //set to 3
  ASSERT_TRUE(h_->getWindow() == 3);
  ASSERT_TRUE(h_->thePacket().window == std::bitset<5> b2(3));

  //test TR
  h_->setTR(1); //set to 1
  ASSERT_TRUE(h_->getTR() == 1);
  ASSERT_TRUE(h_->thePacket().tr == std::bitset<1> b3(1));
  ...
  ASSERT_TRUE(h_->thePacket().seqnum == std::bitset<8> b4(3));

  //test length
  h_->setLength(3); //set to 3
  ASSERT_TRUE(h_->getLength() == 3);
  ASSERT_TRUE(h_->thePacket().length == std::bitset<16> b5(3));

  //test timestamp
  h_->setTimestamp(3); //set to 3
  ASSERT_TRUE(h_->getTimestamp() == 3);
  ASSERT_TRUE(h_->thePacket().timestamp == std::bitset<32> b6(3));

  //test crc1
  h_->setCRC1(3); //set to 3
  ASSERT_TRUE(h_->getCRC1() == 3);
  ASSERT_TRUE(h_->thePacket().crc1 == std::bitset<32> b7(3));

  //test crc2
  h_->setCRC2(3); //set to 3
  ASSERT_TRUE(h_->getCRC2() == 3);
  ASSERT_TRUE(h_->thePacket().crc2 == std::bitset<32> b7(3));
